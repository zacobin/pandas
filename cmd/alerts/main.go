...
import (
	...
	"github.com/cloustone/pandas/alerts/api"
	"github.com/cloustone/pandas/alerts/bcrypt"
	"github.com/cloustone/pandas/alerts/postgres"
...
func main() {
	...
	tracer, closer := initJaeger("alerts", cfg.jaegerURL, logger)
	defer closer.Close()

	dbTracer, dbCloser := initJaeger("alerts_db", cfg.jaegerURL, logger)
...
func loadConfig() config {
	...
	return config{
		...
		alertsFile:    pandas.Env(envRealmsFile, defRealmsFile),
	}
...
func connectToAuthn(cfg config, tracer opentracing.Tracer, logger logger.Logger) (mainflux.AuthNServiceClient, func() error) {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Failed to connect to alerts service: %s", err))
		os.Exit(1)
	}
...
func newService(db *sqlx.DB, tracer opentracing.Tracer, auth mainflux.AuthNServiceClient, c config, logger logger.Logger) alerts.Service {
	//database := postgres.NewDatabase(db)
	//repo := tracing.AlertRepositoryMiddleware(postgres.New(database), tracer)
	hasher := bcrypt.New()

	svc := alerts.New(auth, hasher, nil, nil, nil, nil)
...
func startHTTPServer(tracer opentracing.Tracer, svc alerts.Service, port string, certFile string, keyFile string, logger logger.Logger, errs chan error) {
