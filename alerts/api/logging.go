...
// Alert
func (lm *loggingMiddleware) CreateAlert(ctx context.Context, token string, alert alerts.Alert) (err error) {
...
func (lm *loggingMiddleware) RetrieveAlerts(ctx context.Context, token string) (alertsPage alerts.AlertPage, err error) {
	defer func(begin time.Time) {
	...
	return lm.svc.RetrieveAlerts(ctx, token)
}

// AlertRule
func (lm *loggingMiddleware) CreateAlertRule(ctx context.Context, token string, alertRule alerts.AlertRule) (err error) {
	defer func(begin time.Time) {
		message := fmt.Sprintf("Method register for alert rule %s took %s to complete", alert.Name, time.Since(begin))
		if err != nil {
			lm.logger.Warn(fmt.Sprintf("%s with error: %s.", message, err))
			return
		}
	...
	return lm.svc.CreateAlertRule(ctx, token, alert)
}

func (lm *loggingMiddleware) RetrieveAlertRule(ctx context.Context, token, alertRuleName string) (alertRule alerts.AlertRule, err error) {
	defer func(begin time.Time) {
...
func (lm *loggingMiddleware) UpdateAlertRule(ctx context.Context, token string, alertRule alerts.AlertRule) (err error) {
	defer func(begin time.Time) {
		message := fmt.Sprintf("Method update_alert for alert rule %s took %s to complete", alertRule.Name, time.Since(begin))
		if err != nil {
			lm.logger.Warn(fmt.Sprintf("%s with error: %s.", message, err))
			return
		}
...
func (lm *loggingMiddleware) RetrieveAlertRules(ctx context.Context, token string) (page alerts.AlertRulesPage, err error) {
	defer func(begin time.Time) {
		message := fmt.Sprintf("Method list_alerts took %s to complete", time.Since(begin))
		if err != nil {
			lm.logger.Warn(fmt.Sprintf("%s with error: %s.", message, err))
	...
	return lm.svc.RetrieveAlertRules(ctx, token)
}
