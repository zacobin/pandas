...
func (urm rulechainRepositoryMiddleware) Update(ctx context.Context, rulechain rulechain.RuleChain) (rulechain.RuleChain, error) {
	span := createSpan(ctx, urm.tracer, saveOp)
	defer span.Finish()
	ctx = opentracing.ContextWithSpan(ctx, span)
...
func (urm rulechainRepositoryMiddleware) List(ctx context.Context, UserID string, offset uint64, limit uint64) (rulechain.RuleChainPage, error) {
	span := createSpan(ctx, urm.tracer, listRuleChain)
	defer span.Finish()
	ctx = opentracing.ContextWithSpan(ctx, span)

	return urm.repo.List(ctx, UserID, offset, limit)
}
