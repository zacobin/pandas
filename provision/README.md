...
Also you may use provision service to create certificates for each thing. Each service running on gateway may require more than one thing and channel for communication. Let's say that you are using services [Agent][agent] and [Export](https://gitee.com/pandas/export) on a gateway you will need two channels for `Agent` (`data` and `control`) and one for `Export` and one thing. Additionally if you enabled mtls each service will need its own thing and certificate for access to [Mainflux][mainflux]. Your setup could require any number of things and channels this kind of setup we can call `provision layout`.
...
[mainflux]: https://gitee.com/pandas/mainflux
[bootstrap]: https://gitee.com/pandas/mainflux/tree/master/bootstrap
[export]: https://gitee.com/pandas/export
[agent]: https://gitee.com/pandas/agent
[mfxui]: https://gitee.com/pandas/mainflux/ui