...
func (p *StreamProcessor) execDescribeStream(stmt *xsql.DescribeStreamStatement) (string, error) {
	...
	buff.WriteString("\n")
	util.PrintMap(streamStmt.Options, &buff)
...
func (p *StreamProcessor) DescStream(name string) (*xsql.StreamStmt, error) {
	...
	s, f := p.db.Get(name)
	if !f {
		return nil, util.NewErrorWithCode(util.NOT_FOUND, fmt.Sprintf("Stream %s is not found.", name))
	}
	s1 := s.(string)
...
func NewRuleProcessor(d string) *RuleProcessor {
	processor := &RuleProcessor{
		db:        kvstore.GetKvStore(path.Join(d, "rule")),
		rootDbDir: d,
	}
	return processor
}
...
func (p *RuleProcessor) GetRuleByName(name string) (*api.Rule, error) {
	...
	s, f := p.db.Get(name)
	if !f {
		return nil, util.NewErrorWithCode(util.NOT_FOUND, fmt.Sprintf("Rule %s is not found.", name))
	}
	s1, _ := s.(string)
...
func (p *RuleProcessor) createTopoWithSources(rule *api.Rule, sources []*nodes.SourceNode) (*xstream.TopologyNew, []api.Emitter, error) {
	...
	if selectStmt, err := getStatementFromSql(sql); err != nil {
		...
		store := kvstore.GetKvStore(path.Join(p.rootDbDir, "stream"))
		err = store.Open()
		if err != nil {
			return nil, nil, err
		}
