...
import (
	...
	"github.com/cloustone/pandas/kuiper/xstream/test"
	"os"
	"path"
	"reflect"
	"strings"
...
func compareMetrics(tp *xstream.TopologyNew, m map[string]interface{}) (err error) {
	...
	if util.Config.Basic.Debug == true {
		for i, k := range keys {
...
func sendData(t *testing.T, dataLength int, metrics map[string]interface{}, datas [][]*xsql.Tuple, errCh <-chan error, tp *xstream.TopologyNew, postleap int) error {
	...
	for i := 0; i < dataLength; i++ {
		...
		util.Log.Debugf("Clock add to %d", util.GetNowInMilli())
		time.Sleep(1)
		for _, d := range datas {
			// Make sure time is going forward only
			if d[i].Timestamp > util.GetNowInMilli() {
				mockClock.Set(util.TimeFromUnixMilli(d[i].Timestamp))
				util.Log.Debugf("Clock set to %d", util.GetNowInMilli())
				time.Sleep(1)
			}
			select {
			case err := <-errCh:
	...
	for retry := 100; retry > 0; retry-- {
		...
		if err := compareMetrics(tp, metrics); err == nil {
			...
			util.Log.Debugf("check metrics error at %d: %s", retry, err)
		}
	}
	return nil
}
