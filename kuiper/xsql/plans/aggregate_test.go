...
func TestAggregatePlan_Apply(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			data: xsql.JoinTupleSets{
				...
				xsql.JoinTuple{
					Tuples: []xsql.Tuple{
						{Emitter: "src1", Message: xsql.Message{"id1": 2, "f1": "v2", "ts": util.TimeFromUnixMilli(1568854573431)}},
						{Emitter: "src2", Message: xsql.Message{"id2": 4, "f2": "w3"}},
					},
				},
				xsql.JoinTuple{
					Tuples: []xsql.Tuple{
						{Emitter: "src1", Message: xsql.Message{"id1": 3, "f1": "v1", "ts": util.TimeFromUnixMilli(1568854515000)}},
					},
				},
			},
			result: xsql.GroupedTuplesSet{
	...
	contextLogger := util.Log.WithField("rule", "TestFilterPlan_Apply")
...
func TestAggregatePlanError(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestFilterPlanError")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
		stmt, err := xsql.NewParser(strings.NewReader(tt.sql)).Parse()
		if err != nil {
