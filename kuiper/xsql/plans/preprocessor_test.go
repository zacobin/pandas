...
func TestPreprocessor_Apply(t *testing.T) {
	...
	contextLogger := util.Log.WithField("rule", "TestPreprocessor_Apply")
...
func TestPreprocessorTime_Apply(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			result: &xsql.Tuple{Message: xsql.Message{
				"abc": util.TimeFromUnixMilli(1568854515000),
				"def": util.TimeFromUnixMilli(1568854573431),
			},
			},
		},
		{
		...
		},
		{
			...
			result: &xsql.Tuple{Message: xsql.Message{
				"abc": util.TimeFromUnixMilli(1568894115000),
				"def": util.TimeFromUnixMilli(1568854573431),
			}},
		},
		//Array type
		{
			...
			result: &xsql.Tuple{Message: xsql.Message{
				"a": []time.Time{
					util.TimeFromUnixMilli(1568854515123),
					util.TimeFromUnixMilli(1568854573431),
				},
			},
			},
		},
		{
			...
			result: &xsql.Tuple{Message: xsql.Message{
				"a": map[string]interface{}{
					"b": "hello",
					"c": util.TimeFromUnixMilli(1568854515000),
				},
			},
			},
		},
	...
	defer util.CloseLogger()
	contextLogger := util.Log.WithField("rule", "TestPreprocessorTime_Apply")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
...
func TestPreprocessorEventtime_Apply(t *testing.T) {
	...
	defer util.CloseLogger()
	contextLogger := util.Log.WithField("rule", "TestPreprocessorEventtime_Apply")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
...
func TestPreprocessorError(t *testing.T) {
	...
	defer util.CloseLogger()
	contextLogger := util.Log.WithField("rule", "TestPreprocessorError")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
