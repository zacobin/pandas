...
func TestFilterPlan_Apply(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			data: &xsql.Tuple{
				...
				Message: xsql.Message{
					...
					"def": util.TimeFromUnixMilli(1568853515000),
					"ghi": util.TimeFromUnixMilli(1568854515000),
				},
			},
			result: &xsql.Tuple{
				Emitter: "tbl",
...

		{
			...
			data: &xsql.Tuple{
				Emitter: "tbl",
				Message: xsql.Message{
					"abc": util.TimeFromUnixMilli(1568854515678),
					"def": "hello",
				},
			},
			result: &xsql.Tuple{
	...
	contextLogger := util.Log.WithField("rule", "TestAggregatePlan_Apply")
