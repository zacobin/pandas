...
func TestProjectPlan_Apply1(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			data: &xsql.Tuple{
				...
				Message: xsql.Message{
					"a":  "val_a",
					"ts": util.TimeFromUnixMilli(1568854573431),
				},
			},
			result: []map[string]interface{}{{
		...
		//Schemaless may return a message without selecting column
		{
			...
			data: &xsql.Tuple{
				...
				Message: xsql.Message{
					"a":   "val_a",
					"ts2": util.TimeFromUnixMilli(1568854573431),
				},
			},
			result: []map[string]interface{}{{}},
		},
	...
	contextLogger := util.Log.WithField("rule", "TestProjectPlan_Apply1")
...
func TestProjectPlan_MultiInput(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestProjectPlan_MultiInput")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
		stmt, _ := xsql.NewParser(strings.NewReader(tt.sql)).Parse()
...
func TestProjectPlan_Funcs(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestProjectPlan_Funcs")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
...
func TestProjectPlan_AggFuncs(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestProjectPlan_AggFuncs")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
		stmt, err := xsql.NewParser(strings.NewReader(tt.sql)).Parse()
		if err != nil {
...
func TestProjectPlanError(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestProjectPlanError")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
		stmt, _ := xsql.NewParser(strings.NewReader(tt.sql)).Parse()
