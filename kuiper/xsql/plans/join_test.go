...
func TestLeftJoinPlan_Apply(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			data: xsql.WindowTuplesSet{
				xsql.WindowTuples{
					...
					Tuples: []xsql.Tuple{
						{
							Emitter: "src1",
							Message: xsql.Message{"id1": 1, "f1": "v1", "ts": util.TimeFromUnixMilli(1568854515000)},
						}, {
							Emitter: "src1",
							Message: xsql.Message{"id1": 2, "f1": "v2", "ts": util.TimeFromUnixMilli(1568854525000)},
						}, {
							Emitter: "src1",
							Message: xsql.Message{"id1": 3, "f1": "v3", "ts": util.TimeFromUnixMilli(1568854535000)},
						},
					},
				},
				...
				xsql.WindowTuples{
					...
					Tuples: []xsql.Tuple{
						{
							Emitter: "src2",
							Message: xsql.Message{"id2": 1, "f2": "w1", "ts": util.TimeFromUnixMilli(1568854515000)},
						}, {
							Emitter: "src2",
							Message: xsql.Message{"id2": 2, "f2": "w2", "ts": util.TimeFromUnixMilli(1568854525000)},
						}, {
							Emitter: "src2",
							Message: xsql.Message{"id2": 4, "f2": "w3", "ts": util.TimeFromUnixMilli(1568854545000)},
						},
					},
				},
			},
			result: xsql.JoinTupleSets{
				xsql.JoinTuple{
					Tuples: []xsql.Tuple{
						{Emitter: "src1", Message: xsql.Message{"id1": 1, "f1": "v1", "ts": util.TimeFromUnixMilli(1568854515000)}},
						{Emitter: "src2", Message: xsql.Message{"id2": 1, "f2": "w1", "ts": util.TimeFromUnixMilli(1568854515000)}},
					},
				},
				xsql.JoinTuple{
					Tuples: []xsql.Tuple{
						{Emitter: "src1", Message: xsql.Message{"id1": 2, "f1": "v2", "ts": util.TimeFromUnixMilli(1568854525000)}},
						{Emitter: "src2", Message: xsql.Message{"id2": 2, "f2": "w2", "ts": util.TimeFromUnixMilli(1568854525000)}},
					},
				},
				xsql.JoinTuple{
					Tuples: []xsql.Tuple{
						{Emitter: "src1", Message: xsql.Message{"id1": 3, "f1": "v3", "ts": util.TimeFromUnixMilli(1568854535000)}},
					},
				},
			},
		},
	...
	contextLogger := util.Log.WithField("rule", "TestLeftJoinPlan_Apply")
...
func TestInnerJoinPlan_Apply(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestInnerJoinPlan_Apply")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
		stmt, err := xsql.NewParser(strings.NewReader(tt.sql)).Parse()
		if err != nil {
...
func TestRightJoinPlan_Apply(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestRightJoinPlan_Apply")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
		stmt, err := xsql.NewParser(strings.NewReader(tt.sql)).Parse()
		if err != nil {
...
func TestFullJoinPlan_Apply(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestFullJoinPlan_Apply")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
		stmt, err := xsql.NewParser(strings.NewReader(tt.sql)).Parse()
		if err != nil {
...
func TestCrossJoinPlan_Apply(t *testing.T) {
	...
	fmt.Printf("The test bucket size is %d.\n\n", len(tests))
	contextLogger := util.Log.WithField("rule", "TestCrossJoinPlan_Apply")
	ctx := contexts.WithValue(contexts.Background(), contexts.LoggerKey, contextLogger)
	for i, tt := range tests {
		stmt, err := xsql.NewParser(strings.NewReader(tt.sql)).Parse()
		if err != nil {
