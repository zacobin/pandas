...
func (m Message) Value(key string) (interface{}, bool) {
	...
	if keys := strings.Split(key, COLUMN_SEPARATOR); len(keys) == 1 {
		...
		util.Log.Println("Invalid key: " + key + ", expect source.field or field.")
		return nil, false
	}
...
func getTupleValue(tuple Tuple, t string, key string) (interface{}, bool) {
	...
	default:
		util.Log.Errorf("cannot get tuple for type %s", t)
		return nil, false
	}
}
...
func (jt *JoinTuple) doGetValue(t string, key string) (interface{}, bool) {
	...
	case 1:
		if len(tuples) > 1 {
			...
			util.Log.Infoln("Wrong key: ", key, ", not found")
			return nil, false
		} else {
	...
	case 2:
		...
		return nil, false
	default:
		util.Log.Infoln("Wrong key: ", key, ", expect dot in the expression.")
		return nil, false
	}
}
...
func (v *ValuerEval) simpleDataEval(lhs, rhs interface{}, op Token) interface{} {
	...
	case time.Time:
		rt, err := util.InterfaceToTime(rhs, "")
		if err != nil {
			return invalidOpError(lhs, op, rhs)
		}
		switch op {
