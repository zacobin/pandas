...
func (m *MockSource) Open(ctx api.StreamContext, consumer chan<- api.SourceTuple, _ chan<- error) {
	...
	log.Infof("%d: mock source %s starts", util.GetNowInMilli(), ctx.GetOpId())
	...
	for i, d := range m.data {
		...
		diff := d.Timestamp - util.GetNowInMilli()
		if diff <= 0 {
			log.Warnf("Time stamp invalid, current time is %d, but timestamp is %d", util.GetNowInMilli(), d.Timestamp)
			diff = TIMELEAP
		}
		...
		case <-next:
			...
			log.Debugf("%d: mock source %s is sending data %d:%s", util.TimeToUnixMilli(mockClock.Now()), ctx.GetOpId(), i, d)
			m.Unlock()
		case <-ctx.Done():
