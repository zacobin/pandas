...
func (hps *HTTPPullSource) Configure(device string, props map[string]interface{}) error {
	...
	util.Log.Infof("Initialized with configurations %#v.", hps)
	return nil
}
...
func (hps *HTTPPullSource) initTimerPull(ctx api.StreamContext, consumer chan<- api.SourceTuple, errCh chan<- error) {
	...
	for {
		select {
		case <-ticker.C:
			if resp, e := util.Send(logger, hps.client, hps.bodyType, hps.method, hps.url, hps.headers, true, []byte(hps.body)); e != nil {
