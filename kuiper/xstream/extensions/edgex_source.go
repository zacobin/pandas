...
func (es *EdgexSource) Configure(device string, props map[string]interface{}) error {
	...
	if serviceServer, ok := props["serviceServer"]; ok {
		...
		util.Log.Infof("Connect to value descriptor service at: %s \n", svr)
		...
		if ops1, ok1 := ops.(map[string]interface{}); ok1 {
			for k, v := range ops1 {
				if cv, ok := CastToString(v); ok {
					...
					util.Log.Infof("Cannot convert configuration %s: %s to string type.\n", k, v)
				}
			}
		}
		mbconf.Optional = optional
	}
	util.Log.Infof("Use configuration for edgex messagebus %v\n", mbconf)
...
func (es *EdgexSource) fetchAllDataDescriptors() error {
	if vdArr, err := es.vdc.ValueDescriptors(context.Background()); err != nil {
		...
		if len(vdArr) == 0 {
			util.Log.Infof("Cannot find any value descriptors from value descriptor services.")
		} else {
			util.Log.Infof("Get %d of value descriptors from service.", len(vdArr))
			for i, v := range vdArr {
				util.Log.Debugf("%d: %s - %s ", i, v.Name, v.Type)
			}
		}
	}
	return nil
