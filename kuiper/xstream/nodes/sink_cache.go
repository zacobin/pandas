...
func (c *Cache) initStore(ctx api.StreamContext) {
	...
	if util.Config.Sink.DisableCache {
	...
	c.store = kvstore.GetKvStore(path.Join(dbDir, "sink", ctx.GetRuleId()))
...
func (c *Cache) timebasedRun(ctx api.StreamContext, saveInterval int) {
	logger := ctx.GetLogger()
	c.initStore(ctx)
	ticker := util.GetTicker(saveInterval)
	var tcount = 0
	for {
		select {
		case item := <-c.in:
		...
		case <-ticker.C:
			...
			if (c.changed && l > util.Config.Sink.CacheThreshold) || (tcount == util.Config.Sink.CacheTriggerCount && c.changed) {
			...
			if tcount >= util.Config.Sink.CacheThreshold {
				tcount = 0
			}
		case <-ctx.Done():
