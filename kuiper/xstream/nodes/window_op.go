...
func (o *WindowOperator) execProcessingWindow(ctx api.StreamContext, inputs []*xsql.Tuple, errCh chan<- error) {
	...
	case xsql.TUMBLING_WINDOW:
		o.ticker = util.GetTicker(o.window.Length)
		o.interval = o.window.Length
	case xsql.HOPPING_WINDOW:
		o.ticker = util.GetTicker(o.window.Interval)
	...
	if o.ticker != nil {
		...
		if len(inputs) > 0 && o.triggerTime > 0 {
			nextTick := util.GetNowInMilli() + int64(o.interval)
			next := o.triggerTime
			switch o.window.Type {
	...
	for {
			...
			case *xsql.Tuple:
				...
				case xsql.SESSION_WINDOW:
					if timeoutTicker != nil {
						...
						timeoutTicker = util.GetTimer(o.window.Interval)
		...
		case now := <-timeout:
			if len(inputs) > 0 {
				...
				inputs, _ = o.scan(inputs, util.TimeToUnixMilli(now), ctx)
