...
func (c *Coordinator) Activate() error {
	...
	logger.Infof("Start checkpoint coordinator for rule %s at %d", c.ruleId, util.GetNowInMilli())
	if c.ticker != nil {
		c.ticker.Stop()
	}
	c.ticker = util.GetTicker(c.baseInterval)
	tc := c.ticker.C
	go func() {
		for {
			select {
			case <-tc:
				...
				checkpointId := util.GetNowInMilli()
				...
				for _, r := range c.tasksToTrigger {
					go func(t Responder) {
						if err := t.TriggerCheckpoint(checkpointId); err != nil {
							...
							timeout := util.GetTicker(c.timeout)
							select {
							case <-timeout.C:
