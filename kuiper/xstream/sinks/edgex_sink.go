...
func (ems *EdgexMsgBusSink) Configure(ps map[string]interface{}) error {
	...
	if host, ok := ps["host"]; ok {
		...
		util.Log.Infof("Not find host conf, will use default value '*'.")
	}

	if pro, ok := ps["protocol"]; ok {
		ems.protocol = pro.(string)
	} else {
		util.Log.Infof("Not find protocol conf, will use default value 'tcp'.")
	}
	...
	if port, ok := ps["port"]; ok {
		if pv, ok := port.(float64); ok {
			...
			util.Log.Infof("Not valid port value, will use default value '5563'.")
		}

	} else {
		util.Log.Infof("Not find port conf, will use default value '5563'.")
	}

	if topic, ok := ps["topic"]; ok {
		ems.topic = topic.(string)
	} else {
		util.Log.Infof("Not find topic conf, will use default value 'events'.")
	}

	if contentType, ok := ps["contentType"]; ok {
		ems.contentType = contentType.(string)
	} else {
		util.Log.Infof("Not find contentType conf, will use default value 'application/json'.")
	}
	...
	if ptype, ok := ps["type"]; ok {
		...
		if ems.ptype != messaging.ZeroMQ && ems.ptype != messaging.MQTT && ems.ptype != messaging.RedisStreams {
			util.Log.Infof("Specified wrong message type value %s, will use zeromq messagebus.\n", ems.ptype)
			ems.ptype = messaging.ZeroMQ
		}
	}
	...
	for _, v := range result {
		if m, ok := v[ems.metadata]; ok {
			if m1, ok1 := m.(map[string]interface{}); ok1 {
				...
				util.Log.Infof("Specified a meta field, but the field does not contains any EdgeX metadata.")
			}
		}
	}
	return nil, false
