...
func newField(fis []*fileField) (uis []*field, err error) {
	for _, fi := range fis {
		...
		case []map[string]interface{}:
			var auxFi []*fileField
			if err = util.MapToStruct(t, &auxFi); nil != err {
				return nil, err
			}
...
func (m *Manager) readSinkMetaFile(filePath string) error {
	...
	err := util.ReadJsonUnmarshal(filePath, metadata)
	if nil != err {
	...
	util.Log.Infof("Loading metadata file for sink: %s", finame)
	return nil
}
...
func (this *uiSinks) modifyCustom(uiFields []*field, ruleFields map[string]interface{}) (err error) {
	for i, ui := range uiFields {
		...
		if reflect.Map == reflect.TypeOf(ruleVal).Kind() {
			...
			if err := util.MapToStruct(ruleVal, &auxRuleFields); nil != err {
				return fmt.Errorf(`%s%v %s`, getMsg(this.language, sink, "type_conversion_fail"), err, ui.Name)
			}
			var auxUiFields []*field
			if err := util.MapToStruct(ui.Default, &auxUiFields); nil != err {
				return fmt.Errorf(`%s%v %s`, getMsg(this.language, sink, "type_conversion_fail"), err, ui.Name)
			}
