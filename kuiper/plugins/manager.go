...
func NewPluginManager() (*Manager, error) {
	var outerErr error
	once.Do(func() {
		dir, err := util.GetLoc("/plugins")
		if err != nil {
			outerErr = fmt.Errorf("cannot find plugins folder: %s", err)
			return
		}
		etcDir, err := util.GetLoc("/etc")
		if err != nil {
		...
		if err := singleton.readSourceMetaDir(); nil != err {
			util.Log.Errorf("readSourceMetaDir:%v", err)
		}
		if err := singleton.readSinkMetaDir(); nil != err {
			util.Log.Errorf("readSinkMetaDir:%v", err)
		}
		if err := singleton.readFuncMetaDir(); nil != err {
			util.Log.Errorf("readFuncMetaDir:%v", err)
		}
		if err := singleton.readUiMsgDir(); nil != err {
			util.Log.Errorf("readUiMsgDir:%v", err)
		}
	})
...
func getSoFilePath(m *Manager, t PluginType, name string) (string, error) {
	v, ok := m.registry.Get(t, name)
	if !ok {
		return "", util.NewErrorWithCode(util.NOT_FOUND, fmt.Sprintf("invalid name %s: not exist", name))
	}
	...
	if _, err := os.Stat(p); err != nil {
		return "", util.NewErrorWithCode(util.NOT_FOUND, fmt.Sprintf("cannot find .so file for plugin %s", name))
	}
	return p, nil
}
...
func (m *Manager) install(t PluginType, name, src string, shellParas []string) ([]string, string, error) {
	...
	for _, file := range r.File {
		...
		if yamlFile == fileName {
			...
			if err := unzipTo(file, jsonPath); nil != err {
				util.Log.Errorf("Failed to decompress the metadata %s file", fileName)
			} else {
	...
	if len(filenames) != expFiles {
		...
		if err != nil {
			...
			util.Log.Infof(`err:%v stdout:%s stderr:%s`, err, outb.String(), errb.String())
			return filenames, "", err
		} else {
			util.Log.Infof("install %s plugin %s", PluginTypes[t], name)
		}
	}
...
func downloadFile(filepath string, uri string) error {
	util.Log.Infof("Start to download file %s\n", uri)
