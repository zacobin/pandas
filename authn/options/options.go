...
func NewServingOptions() *ServingOptions {
	s := ServingOptions{
		...
		RealmConfigFile:  "./authn/authn-realms.json",
		RolesFile:        "./authn/authn-roles.json",
...
func (s *ServingOptions) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&s.ServiceID, "service-id", s.ServiceID, "authn service ID")
	fs.StringVar(&s.InitFile, "init-file", s.InitFile, "initial authn config file")
